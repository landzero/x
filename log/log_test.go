...
const (
	...
	Rline         = `(59|61): ` // must update if the calls to l.Printf / l.Print below move
...
var tests = []tester{
	...
	// everything at once:
	{Ldate | Ltime | Lmicroseconds | Llongfile, "XXX", "\\[" + Rdate + " " + Rtime + Rmicroseconds + "\\] XXX" + Rlongfile},
	{Ldate | Ltime | Lmilliseconds | Lshortfile, "XXX", "\\[" + Rdate + " " + Rtime + Rmilliseconds + "\\] XXX" + Rshortfile},
}
...
func TestFlagAndPrefixSetting(t *testing.T) {
	...
	l.Print("hello")
	pattern := "^\\[" + Rdate + " " + Rtime + Rmilliseconds + "\\] Reality:hello\n"
	matched, err := regexp.Match(pattern, b.Bytes())
	if err != nil {
		t.Fatalf("pattern %q did not compile: %s", pattern, err)
	}
	if !matched {
		t.Error("message did not match pattern, expect", pattern, "got", string(b.Bytes()))
	}
}
...
func TestUTCFlag(t *testing.T) {
	...
	now := time.Now().UTC()
	l.Print("hello")
	want := fmt.Sprintf("[%d/%.2d/%.2d %.2d:%.2d:%.2d] Test:hello\n",
